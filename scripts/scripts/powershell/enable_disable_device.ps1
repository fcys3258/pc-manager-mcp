# enable_disable_device.ps1
# V1.1.0 L1 Skill - Enable or disable a device
...
try {
    ...
    if (-not $Parameter -or -not $Parameter.PSObject.Properties['pnp_device_id']) {
        Emit-Error 'INVALID_ARGUMENT' 'Missing required parameter: pnp_device_id' $false
        return
    }
    if (-not $Parameter.PSObject.Properties['action']) {
        Emit-Error 'INVALID_ARGUMENT' 'Missing required parameter: action' $false
        return
    }
    
    $pnpDeviceId = $Parameter.pnp_device_id
    $action = $Parameter.action
    
    # Validate action value
    if ($action -ne 'enable' -and $action -ne 'disable') {
        Emit-Error 'INVALID_ARGUMENT' "Invalid action '$action'. Must be 'enable' or 'disable'." $false
        return
    }
    ...
    if ($device.Count -eq 0) {
        Emit-Error 'TARGET_NOT_FOUND' "Device with PnP ID '$pnpDeviceId' not found." $false
        return
    }
    $device = $device[0]
    if (-not $device) {
    ...
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'
            would_perform_action = "$action device '$($device.FriendlyName)'" 
        }
        return
    }
    ...
    try {
        if ($action -eq 'enable') {
            ...
            Emit-Success @{ result = 'enabled'; reason = "Device '$($device.FriendlyName)' enabled." }
        } else {
            Disable-PnpDevice -InstanceId $pnpDeviceId -Confirm:$false -ErrorAction Stop
            Emit-Success @{ result = 'disabled'; reason = "Device '$($device.FriendlyName)' disabled." }
        }
    } catch {
        Emit-Error 'ACTION_FAILED' "Failed to $action device '$($device.FriendlyName)'. Reason: $($_.Exception.Message)" $false
    }
