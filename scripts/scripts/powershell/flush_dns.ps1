# flush_dns.ps1
# V1.1.0 L1 Skill - Flush DNS Client Cache
...
try {
    ...
    $BudgetMs = if ($script:SkillArgs.metadata -and $script:SkillArgs.metadata.PSObject.Properties['timeout_ms']) { $script:SkillArgs.metadata.timeout_ms } else { 0 }
    ...
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'
            would_perform_action = "Flush DNS Client Cache"
        }
        return
    }
    ...
    $rawOutput = ""
    try {
        # Primary path: Clear-DnsClientCache
        Clear-DnsClientCache
    } catch {
        # Fallback path: ipconfig /flushdns
        $fallback = $true
        ...
        $rawOutput = ipconfig /flushdns
        [Console]::OutputEncoding = $prevEncoding
    }
    
    # 7. Success output
    Emit-Success @{ result = 'flushed' } @{ fallback_mode = $fallback; raw_output = $rawOutput }
