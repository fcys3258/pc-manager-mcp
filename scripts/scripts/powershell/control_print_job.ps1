# control_print_job.ps1
# V1.1.0 L1 Skill - Control print job (pause/resume/restart)
...
try {
    ...
    # Validate parameters
    $printerName = if ($Parameter -and $Parameter.PSObject.Properties['printer_name']) { $Parameter.printer_name } else { $null }
    $jobId = if ($Parameter -and $Parameter.PSObject.Properties['job_id']) { $Parameter.job_id } else { $null }
    $action = if ($Parameter -and $Parameter.PSObject.Properties['action']) { $Parameter.action } else { $null }
    ...
    if (-not $jobId) {
        Emit-Error 'INVALID_ARGUMENT' 'Parameter "job_id" is required.' $false
        return
    }
    if (-not $action) {
        Emit-Error 'INVALID_ARGUMENT' 'Parameter "action" is required (pause/resume/restart).' $false
        return
    }
    
    # Validate action before dry_run check
    $validActions = @("pause", "suspend", "resume", "restart")
    if ($action.ToLower() -notin $validActions) {
        Emit-Error 'INVALID_ACTION' "Invalid action: $action (valid: pause/resume/restart)" $false
        return
    }
    ...
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'
            would_perform_action = "$action print job $jobId on printer '$printerName'"
        }
        return
    }

    # 4. Core logic - Control print job
    $message = ""
    ...
    try {
        switch ($action.ToLower()) {
            ...
            "restart" {
                Restart-PrintJob -PrinterName $printerName -ID $jobId -ErrorAction Stop
                $message = "Print job restarted"
            }
            default {
                Emit-Error 'INVALID_ACTION' "Invalid action: $action (valid: pause/resume/restart)" $false
                return
        ...
        Emit-Error 'ACTION_FAILED' "Failed to $action print job: $($_.Exception.Message)" $false
    }
