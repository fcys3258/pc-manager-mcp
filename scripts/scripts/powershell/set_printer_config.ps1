# set_printer_config.ps1
# V1.1.0 L1 Skill - Set printer configuration
...
try {
    ...
    $paperSize = if ($Parameter.PSObject.Properties['paper_size']) { $Parameter.paper_size } else { $null }
    $color = if ($Parameter.PSObject.Properties['color']) { $Parameter.color } else { $null }
    $duplex = if ($Parameter.PSObject.Properties['duplex']) { $Parameter.duplex } else { $null }

    $dryRun = if ($Parameter.PSObject.Properties['dry_run']) { $Parameter.dry_run } else { $false }
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'
            would_perform_action = "Set printer configuration for '$printerName'"
        }
        return
    }

    # 4. Core logic - Set printer configuration
    try {
        ...
        if ($paperSize) { 
            ...
            $appliedSettings += "paper_size=$paperSize"
        }
        if ($null -ne $color) { 
            $params['Color'] = [bool]$color 
            $appliedSettings += "color=$color"
        }
        if ($duplex) { 
            $params['DuplexingMode'] = $duplex 
            $appliedSettings += "duplex=$duplex"
        }
        
        if ($appliedSettings.Count -eq 0) {
            Emit-Error 'NO_SETTINGS' "No settings provided to apply." $false
            return
        }
        ...
        Emit-Error 'CONFIG_FAILED' "Failed to set printer configuration: $($_.Exception.Message)" $false
    }
