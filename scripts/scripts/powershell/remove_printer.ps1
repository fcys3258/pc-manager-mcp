# remove_printer.ps1
# V1.1.0 L1 Skill - Remove printer
...
try {
    ...
    if (-not $printerName) {
        Emit-Error 'INVALID_ARGUMENT' 'Parameter "name" is required.' $false
        return
    }
    ...
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'
            would_perform_action = "Remove printer '$printerName'"
        }
        return
    }

    # 4. Core logic - Remove printer
    try {
        ...
        Emit-Error 'PRINTER_NOT_FOUND' "Printer not found: $printerName" $false
    } catch {
        Emit-Error 'REMOVE_FAILED' "Failed to remove printer: $($_.Exception.Message)" $false
    }
