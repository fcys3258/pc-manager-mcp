...
# V1.1.0 L1 Skill - Reset Network Stack
...
try {
    ...
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'; 
            would_perform_action = "Reset Network Stack (netsh winsock reset && netsh int ip reset)" 
        }
        return
    }
    ...
    $winsockOutput = netsh winsock reset
    
    if (-not (Test-Budget $__sw $BudgetMs)) {
        [Console]::OutputEncoding = $prevEncoding
        Emit-Error 'TIME_BUDGET_EXCEEDED' "Budget exceeded after winsock reset." $true
        return
    }

    $ipResetOutput = netsh int ip reset
    [Console]::OutputEncoding = $prevEncoding

    # 7. Success output
    Emit-Success @{ result = 'completed' } @{ reboot_required = 'likely'; raw_output_winsock = $winsockOutput; raw_output_ipreset = $ipResetOutput }
