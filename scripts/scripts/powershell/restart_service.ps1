# restart_service.ps1
# V1.1.0 L1 Skill - Restart a Windows service
...
try {
    ...
    $timeoutSeconds = 55 # Slightly less than python side's 60s
    ...
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'
            would_perform_action = "Restart service '$serviceName'"
    ...
    # Check budget
    if (-not (Test-Budget $__sw $BudgetMs)) {
        Emit-Error 'TIME_BUDGET_EXCEEDED' "Budget exceeded before restarting service." $true
        return
    }
    ...
    try {
        ...
        # Check budget
        if (-not (Test-Budget $__sw $BudgetMs)) {
            Emit-Error 'TIME_BUDGET_EXCEEDED' "Budget exceeded after stopping service." $true
            return
        }
        ...
        Emit-Success @{ result = 'restarted'; reason = "Service '$serviceName' restarted successfully." }
    } catch {
        # Catch exceptions from Restart-Service, Stop-Service, Start-Service or WaitForStatus
        Emit-Error 'ACTION_FAILED' "Failed to restart service '$serviceName'. Reason: $($_.Exception.Message)" $false
    }
