# force_gpupdate.ps1
# V1.1.0 L1 Skill - Force refresh Group Policy
...
try {
    ...
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'
            would_perform_action = "Force refresh Group Policy (gpupdate /force /wait:0)"
        }
        return
    }
    ...
    # Use /wait:0 to return immediately
    $gpupdateOutput = gpupdate.exe /force /wait:0 2>&1
    [Console]::OutputEncoding = $prevEncoding
    
    $rawOutput = @($gpupdateOutput | ForEach-Object { $_.ToString() })
    
    # Check results
    $userPolicyUpdated = $false
    $computerPolicyUpdated = $false
    
    foreach ($line in $rawOutput) {
        if ($line -match 'User Policy update has completed successfully') {
            $userPolicyUpdated = $true
        }
        if ($line -match 'Computer Policy update has completed successfully') {
    ...
    Emit-Success @{
        result = 'initiated'
        user_policy_updated = $userPolicyUpdated
        computer_policy_updated = $computerPolicyUpdated
        raw_output = $rawOutput
        note = "gpupdate /force /wait:0 was executed. Full policy application may take a few seconds."
    }
