# start_service.ps1
# V1.1.0 L1 Skill - Start a Windows service
...
try {
    ...
    if ($service.Status -eq 'Running') {
        Emit-Success @{ result = 'already_running'; reason = "Service '$serviceName' is already running." }
        return
    }
    ...
    if ($dryRun) {
        Emit-Success @{ 
            result = 'dry_run'; 
            would_perform_action = "Start service '$serviceName'" 
        }
        return
    }
    ...
    try {
        ...
        # Wait for service status to become Running with timeout
        $service.WaitForStatus('Running', [TimeSpan]::FromSeconds(25))
        Emit-Success @{ result = 'started'; reason = "Service '$serviceName' started successfully." }
    } catch {
        Emit-Error 'ACTION_FAILED' "Failed to start service '$serviceName'. Reason: $($_.Exception.Message)" $false
    }
